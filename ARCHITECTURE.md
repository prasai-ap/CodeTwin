...
7. Classifies impacted files as tests, API files, or source files using path and AST heuristics, and derives components from paths.
...
Graph nodes represent Python files. Analyzer edges point from an importer to the dependency it imports. For a changed dependency, downstream impact travels in the reverse direction, from that dependency to its importers. The UI lays out the predicted subset and draws the impact direction so reviewers can follow change propagation. Node appearance may show Bob's assessment, but the graph's predicted membership remains based on the deterministic analyzer.
...
`examples/ecommerce/` is a synthetic FastAPI shop with authentication, users, products, orders, payments, notifications, route modules, an in-memory database/repository layer, and tests. The regression patch divides `amount_cents` by 100 before storing the payment, even though the value is already in cents. The checkout workflow test detects the mismatch. The correction demo restores the cents value in a new captured revision; Bob and the targeted test must run again.
